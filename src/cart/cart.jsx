        ...
        <div className="max-w-6xl mx-auto">
              ...
              return (
                ...
                <div key={elem.id} className="grid grid-cols-12 gap-4 px-8 py-6 border-b items-center">
                  <div className="col-span-5 flex items-center gap-4">
                    <img src={`http://37.27.29.18:8002/images/${elem?.product?.image}`} alt="LCD Monitor" className="w-14 h-14 rounded" />
                    <span className="font-medium">{elem?.product?.productName}</span>
                  </div>
                  <div className="col-span-2 text-center text-gray-600">${elem?.product?.price}</div>
                  <div className="col-span-3 flex justify-center">
                    <div className="flex items-center border border-gray-300 rounded-md w-24">
                      <button onClick={() => dispatch(minus(elem.id))} className="px-3 py-2 text-gray-600 hover:bg-gray-100">-</button>
                      <span className="px-4 py-2 w-12 text-center font-medium">{elem.quantity}</span>
                      <button onClick={() => dispatch(plus(elem.id))} className="px-3 py-2 text-gray-600 hover:bg-gray-100">+</button>
                    </div>
                  </div>
                  <div className="col-span-2 text-right font-semibold">${elem?.product?.price * elem.quantity}</div>
                  <button onClick={() => dispatch(deleted(elem.id))} className="justify-self-end text-red-500 hover:text-red-700 text-2xl">×</button>
                </div>
              )
            })}
          </div>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 my-10">
            ...
            <div className="flex gap-4">
              ...
              <button onClick={() => dispatch(deleteded())} className="px-8 py-3 border border-red-500 text-red-500 rounded-md hover:bg-red-50">
                Remove all
              </button>
            </div>
          </div>
