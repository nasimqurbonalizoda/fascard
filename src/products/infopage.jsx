...
const Infopage = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 font-sans bg-white">
      <button 
        onClick={() => navigate(-1)} 
        className="mb-8 text-red-600 hover:text-red-700 font-medium flex items-center gap-2"
      >
        Back
      </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-20">
        <div className="flex flex-col-reverse lg:flex-row gap-6">
          <div className="flex lg:flex-col gap-4">
            {dataByid?.images?.slice(0, 4).map((img, i) => (
              <div key={i} className="w-20 h-20 bg-gray-100 rounded-lg border-2 border-transparent hover:border-blue-500 transition">
                <img src={`http://37.27.29.18:8002/images/${img.images}`} className="w-full h-full object-contain p-2" />
              </div>
            ))}
          </div>
          ...
          <div className="relative bg-gray-50 rounded-2xl p-12 group">
            {dataByid?.images && dataByid.images.length > 0 ? (
              <img
                src={`http://37.27.29.18:8002/images/${dataByid.images[0].images}`}
                alt={dataByid?.productName}
                className="max-w-full max-h-96 object-contain"
              />
            ) : (
              // <img src={den} className="max-w-full max-h-96 object-contain" />
              []
            )}
            {dataByid?.hasDiscount && (
              <div className="absolute top-4 left-4 bg-red-500 text-white px-3 py-1 rounded text-sm font-bold">
                -{Math.round(((dataByid.price - dataByid.discountPrice) / dataByid.price) * 100)}%
              </div>
            )}
        ...
        <div className="flex flex-col justify-center space-y-6">
          ...
          <div className="flex items-center gap-4 text-lg">
            ...
            <span className={`font-semibold ${dataByid?.quantity > 0 ? 'text-green-600' : 'text-red-600'}`}>
              {dataByid?.quantity > 0 ? 'In Stock' : 'Out of Stock'}
            </span>
          </div>
          ...
          <div className="text-3xl font-bold">
            {dataByid?.hasDiscount ? (
              <>
                <span className="text-red-600">{dataByid.discountPrice}$</span>
                <span className="text-gray-400 line-through ml-4">{dataByid.price}$</span>
              </>
            ) : (
              `${dataByid?.price} $`
            )}
          ...
          <p className="text-gray-600 leading-relaxed">
            {dataByid?.description || "PlayStation 5 Controller Skin High quality vinyl with air channel adhesive for easy bubble free install & mess free removal Pressure sensitive."}
          </p>
          ...
          <div className="border-t border-gray-300 pt-6 space-y-8">
            ...
            <div className="flex items-center gap-4">
              <div className="flex border border-gray-300 rounded overflow-hidden">
                ...
                <span className="px-8 py-3 font-semibold">1</span>
                <button className="px-5 py-3 hover:bg-red-500 hover:text-white">+</button>
              </div>
